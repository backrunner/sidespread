...
本工具的目标是：**检测这种高频缺损并补偿，使 mid / side 信号在频谱与质量上对齐**，提升 AI 生成音乐的立体声高频表现。
...
1. **高频能量比** `R_hf = E_S(f>f_c) / E_M(f>f_c)`，`f_c` 默认 8 kHz（可配）。`R_hf` 持续低于阈值（默认 0.3）→ 缺损。
...
| `R_hf` 缺损 + `corr_hf > 0.6` | **A（DSP）** | mid→side 延展，廉价保真 |
| `R_hf` 缺损 + `corr_hf < 0.4` | **B（神经）** | side 自身中频向高频外推，用 UniverSR |
| 中间区间 | **A + B 混合** | A 为主、B 微调，或分段分别走 |

**整体判定**：若全曲 `R_hf` 均正常 → 提示用户"该音频不需要处理"并退出（仅输出检测报告）。

### F3 — A 路线：DSP 补偿（mid → side 延展）
当 mid 与 side 在中频近似时，把 mid 的高频能量"借给" side：
- STFT（n_fft=4096/8192，hop=1024，Hann 窗）。
- 对 M 高频 bin 取幅度，按"side 中频能量 / mid 高频能量"比例缩放，写入 S 高频 bin。
- 相位：用 M 相位 + 微小抖动（±5°~30°）或 Hilbert ±90° 偏移，避免完全同相伪影。
- 交叉淡入（cutoff 附近过渡带平滑混合）。
- 纯 Rust 实现，无外部依赖。

### F4 — B 路线：神经网络补偿（中频→高频延展）
当 mid 与 side 不近似（side 是独立内容）时，用 **UniverSR**（arXiv:2510.00771，MIT，音乐训练，229MB，vocoder-free flow matching，iSTFT 直出）把 side 自身中频向高频延展：
...
### N3 — 可分发
- 单一 Rust 二进制 + 一个 ONNX 模型文件（229 MB，可选下载）。
- 不捆绑 PyTorch / CUDA。
...
```
sidespread process <input.wav> [-o <output.wav>] [--mode auto|dsp|nn|skip]
                  [--fc 8000] [--rhf-threshold 0.3] [--corr-threshold 0.6,0.4]
                  [--ode-steps 4] [--report report.json] [--model-path <onnx>]
...
2. 对需要处理的音频，`sidespread process` 输出修复后的 WAV，`report.json` 显示 LSD 下降、R_hf 趋近 1、ICCC 合理。
3. 对不需要处理的音频，`process` 提示用户并仅输出检测报告。
4. A 路线（DSP）纯 Rust 实现，无外部模型。
5. B 路线（UniverSR ONNX）在 CPU 上能完成推理，输出与 Python 参考实现数值误差 < 1e-3。
6. `sidespread eval` 在对照集上 SNR / LSD-ground-truth 量化改善。
